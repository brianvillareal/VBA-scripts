Attribute VB_Name = "LoadedRT999"
Option Explicit
          
    Dim strArchive As String, _
        strButton  As String, _
        strMessage As String, _
        strRT999   As String, _
        wbkArchive As Workbook, _
        wbkRecon   As Workbook, _
        wksRecon   As Worksheet

Private Sub InitLoadedRT999(control As IRibbonControl)
    
    strButton = Replace(control.ID, "ReconRT999", vbNullString)
    Call SetAthenaType
    Application.ScreenUpdating = False
    Set wbkArchive = OpenFileWait(strArchive)
    strArchive = vbNullString
    Set wbkRecon = OpenFileWait(strRT999)
    strRT999 = vbNullString
    Set wksRecon = wbkRecon.Worksheets(1)
    Call ImportBalance("D", "F", "H")
...
Private Function SetAthenaType()
    ...
    strRT999 = strReports & "RT999 " & strButton & ".xls"
...
Private Sub ShowMessage()
    ...
    MsgBox "Load balances imported from " & strButton & " RT999 reports." & vbNewLine & _
           vbNewLine & _
